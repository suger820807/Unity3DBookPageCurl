...
public class BookPro : MonoBehaviour
{
    ...
    public void ReleasePage()
    {
        if (pageDragging)
        {
            ...
            Vector3 new_ebr = new Vector3(BookPanel.rect.width/2, -BookPanel.rect.height / 2);
            Vector3 new_ebl = new Vector3(0, -BookPanel.rect.height / 2);
            float distanceToLeft = Vector2.Distance(c, new_ebl);
            float distanceToRight = Vector2.Distance(c, new_ebr);

            Debug.Log("distanceToLeft=" + distanceToLeft);
            Debug.Log("distanceToRight=" + distanceToRight);
            Debug.Log("new_ebr=" + new_ebr);
            Debug.Log("new_ebl=" + new_ebl);
